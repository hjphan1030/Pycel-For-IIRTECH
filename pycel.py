...
# Load the replacements from the db_wildcard.txt file
with open('db_wildcard.txt', 'r', encoding='utf-8') as f:
    db_wildcard_lines = f.read().splitlines()
    db_wildcard_replacements = dict(zip(db_wildcard_lines[::2], db_wildcard_lines[1::2]))
...
for file_index, filename in enumerate(files):
    ...
    # Iterate over the cells and replace strings
    ...
    # Iterate over the cells and replace strings using wildcards
    for row in sheet.iter_rows():
        for cell in row:
            if isinstance(cell.value, str):
                for old, new in db_wildcard_replacements.items():
                    # Convert Excel wildcard to regex
                    old = excel_to_regex(old)
                    cell.value = re.sub(old, new, cell.value)
